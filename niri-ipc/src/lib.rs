...
pub enum Event {
    ...
    GestureProgress {
        ...
        ///   natural-scroll-adjusted) finger delta on the dominant axis, normalized by
        ///   `swipe-progress-distance` (default 200 px for touchscreen, 40 libinput units for
        ///   touchpad — same knob name, separate config block).
        /// - Pinches use `(current_spread - start_spread) / pinch-progress-distance` (default 100
        ///   px) — an absolute measurement (not accumulated), so pinching in then out returns
        ///   progress cleanly to near 0 with no float drift. Positive = pinch-out, negative =
        ///   pinch-in.
        /// - Rotations use `cumulative_rotation / rotation-progress-angle` (default 90°). Positive
        ///   = counter-clockwise.
        progress: f64,
